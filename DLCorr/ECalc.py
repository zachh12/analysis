...
cal_dict = {
626: [[0.4079163772696832, 88.24274370714352],[0.4087940881749243, 124.10630773102548],[0.408892157679714, -3.1822542545719443], [0.3575423871894163, 350.83971830570977]],
672: [[0.4079163772696832, 88.24274370714352],[0.4087940881749243, 124.10630773102548],[0.408892157679714, -3.1822542545719443], [0.408892157679714, -3.1822542545719443]],
692: [[0.40208432759502305, 94.28843463439557], [0.4025818080976151, 131.02582584581228], [0.40226129361430035, 0.9147482366054192], [0.408892157679714, -3.1822542545719443]]
}
chan = 626
def main():
    calcEnergy()
...
def calibrate():
    ...
    df = pd.read_hdf("data/trap_chan" + str(chan) + "data.h5", key='data')

    df_max2614 = df[(df['trap_max'] > 6000) & (df['trap_max'] < 6600)]
    df_max2614mean = np.mean(df_max2614['trap_max'])
    df_max860 = df[(df['trap_max'] > 1700) & (df['trap_max'] < 2000)]
    df_max860mean = np.mean(df_max860['trap_max'])
    df_max727 = df[(df['trap_max'] > 1300) & (df['trap_max'] < 1700)]
    ...
    df_max860 = df[(df['trap_ft'] > 1600) & (df['trap_ft'] < 2000)]
    df_max860mean = np.mean(df_max860['trap_ft'])
    df_max727 = df[(df['trap_ft'] > 1200) & (df['trap_ft'] < 1600)]
    ...
    df_max860 = df[(df['fitE'] > 2001) & (df['fitE'] < 2500)]
    ...
    df_max2614 = df[(df['trap_raw'] > 6000) & (df['trap_raw'] < 6300)]
    df_max2614mean = np.mean(df_max2614['trap_raw'])
    df_max860 = df[(df['trap_raw'] > 1905) & (df['trap_raw'] < 2200)]
    df_max860mean = np.mean(df_max860['fitE'])
    df_max727 = df[(df['trap_raw'] > 0) & (df['trap_raw'] < 1900)]
    df_max727mean = np.mean(df_max727['trap_raw'])
    adc = [df_max727mean, df_max860mean, df_max2614mean]
    slope, intercept, r_value, p_value, std_err = stats.linregress(adc, cal)
    print(slope, intercept)
    df['trap_raw'] = df['trap_raw'] * cal_dict[chan][3][0] + cal_dict[chan][3][1]
    
    print(np.std(df['trap_max']) * 2.35, np.std(df['trap_ft']) * 2.35, np.std(df['fitE']) * 2.35, np.std(df['trap_raw']) * 2.35, np.std(df['ecal']) * 2.35)

    df.to_hdf("data/trapC_chan" + str(chan) + "data.h5", key='data')
